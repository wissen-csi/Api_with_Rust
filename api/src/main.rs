...
use api::{db::{self, AppContext}, dto::user_p::User};
use axum::{Json, extract::State, response::IntoResponse};
use sea_orm::{ActiveValue, Database, EntityTrait, sqlx::types::chrono};
...
async  fn main() {
    ...
    let router = axum::Router::new().route("/api", axum::routing::post(axum::routing::post(create_user)).with_state(ctx));
    axum::serve(listener,router).await.unwrap_or_else(|err|{
        panic!("{}",err);
    });
}
pub async fn create_user(State(ctx): State<AppContext>, Json(user): Json<User>) -> impl IntoResponse {    
